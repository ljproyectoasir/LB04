...
ORIGENES=( /etc/rc.local /etc/init.d/ /etc/ha.d/ /etc/haproxy/ /etc/bind/ /var/cache/bind/ /etc/redis/ /etc/nagios-nrpe/ /etc/network/interfaces /etc/resolv.conf /var/log/ /root/scripts/ )

# Destinos del backup (array) dentro de /backups/daily/
DESTINOS=( rc.local servicios heartbeat haproxy bind bind-zone redis nrpe conf-ip resolver logs scripts )
